...
export class FeedService {
  ...
  createPost(feedPost: FeedPost, user: User): Observable<FeedPost> {
    ...
    console.log((feedPost.author = user));
    console.log(feedPost.author);
    console.log(user);
    const a = from(this.feedPostRepository.save(feedPost));
    return a;
  }
  ...
  findPostById(id: number): Observable<FeedPost> {
    const response = from(
      this.feedPostRepository
        .findOne({ where: { id: id }, relations: { author: true } })
        .then((posts) => {
          return <FeedPost>posts;
        }),
    );
